...
public:
    TurtleFollower() : Node("turtle_follower")
    {
        ...
        turtle1_pose_subscriber_ = this->create_subscription<turtlesim::msg::Pose>(
            "/turtle1/pose", 10, std::bind(&TurtleFollower::turtle1PoseCallback, this, std::placeholders::_1));

        // turtle2의 위치를 구독
        turtle2_pose_subscriber_ = this->create_subscription<turtlesim::msg::Pose>(
            "/turtle2/pose", 10, std::bind(&TurtleFollower::turtle2PoseCallback, this, std::placeholders::_1));
...
private:
    ...
    void followTurtle1()
    {
        ...
        // 거리 기준 임계값 (turtle2가 turtle1과 너무 가까우면 멈추게 함)
